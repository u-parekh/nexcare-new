...
var HTML_TAGS = [
  ...
  "video",
  "wbr",
  // Deprecated/legacy but still parsed as host elements
  "acronym",
  "big",
  "center",
  "dir",
...
// src/babel-plugin/index.ts
var R3F_IMPORT_PREFIX = "@react-three/";
var babelMetadataPlugin = ({ types: t }) => {
  ...
  const moduleImportsR3F = (programPath) => programPath.node.body.some(
    (node) => t.isImportDeclaration(node) && typeof node.source?.value === "string" && node.source.value.startsWith(R3F_IMPORT_PREFIX)
  );
  const isCustomReconcilerIntrinsic = (openingEl) => {
    const nameNode = openingEl?.name;
    if (!nameNode || !t.isJSXIdentifier(nameNode)) return false;
    const name = nameNode.name;
    if (!/^[a-z]/.test(name)) return false;
    if (name.includes("-")) return false;
    return !HOST_TAGS.has(name);
  };
  return {
    name: "element-metadata-plugin",
    visitor: {
      // Decide the per-module skip up front — Babel runs Program enter ahead of
      // the JSX visitors below.
      Program(programPath, state) {
        state.skipVisualEdits = moduleImportsR3F(programPath);
      },
      JSXElement(jsxPath, state) {
        if (state.skipVisualEdits) return;
        if (isCustomReconcilerIntrinsic(jsxPath.node.openingElement)) return;
        return jsxElementVisitor.call(this, jsxPath, state);
      },
      JSXOpeningElement(openingPath, state) {
        if (state.skipVisualEdits) return;
        if (isCustomReconcilerIntrinsic(openingPath.node)) return;
        return jsxOpeningElementVisitor.call(this, openingPath, state);
      }
    }
  };
};
