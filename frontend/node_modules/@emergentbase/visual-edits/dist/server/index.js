...
function getCodeServerPassword() {
  try {
    const conf = import_fs.default.readFileSync(
...
var SUP_PASS = getCodeServerPassword() || process.env.VISUAL_EDIT_API_KEY || null;
...
function findFileRecursive(dir, filename) {
  try {
    ...
    for (const file of files) {
      ...
      if (file.isDirectory()) {
        if (file.name === "node_modules" || file.name === "public" || file.name === ".git" || file.name === "build" || file.name === "dist" || file.name === "coverage") {
          continue;
        }
...
function getRelativePath(absolutePath, frontendRoot) {
  const rel = import_path.default.relative(frontendRoot, absolutePath);
  return "/" + rel;
}
function validateFilePath(targetFile, frontendRoot, fileName) {
  const normalizedTarget = import_path.default.normalize(targetFile);
...
// src/server/variable-edit.ts
var import_fs4 = __toESM(require("fs"));
var import_path3 = __toESM(require("path"));

// src/server/ast-helpers.ts
var import_fs3 = __toESM(require("fs"));
var import_path2 = __toESM(require("path"));
function resolveSourceFile(source, frontendRoot) {
  let base;
  if (source.startsWith("@/")) {
    base = import_path2.default.join(frontendRoot, "src", source.slice(2));
  } else if (source.startsWith("./") || source.startsWith("../")) {
    base = import_path2.default.resolve(frontendRoot, "src", source);
  } else {
    return null;
  }
  for (const ext of EXTENSIONS) {
    const file = base.endsWith(ext) ? base : base + ext;
    if (import_fs3.default.existsSync(file) && import_fs3.default.statSync(file).isFile()) {
      return file;
    }
  }
  if (import_fs3.default.existsSync(base) && import_fs3.default.statSync(base).isDirectory()) {
    for (const ext of EXTENSIONS) {
      const indexFile = import_path2.default.join(base, "index" + ext);
  ...
  if (typeof value === "object") {
    return t.objectExpression(
      Object.entries(value).map(
        ([k, v]) => t.objectProperty(t.identifier(k), jsonToAst(v, t))
      )
    );
  }
  return t.nullLiteral();
}
function parseJsxChildren(content, parser, t) {
  if (content === void 0) {
    return null;
  }
...
function processVariableEdit(change, frontendRoot, babelTools) {
  ...
  const normalizedTarget = import_path3.default.normalize(targetFile);
  ...
  const content = import_fs4.default.readFileSync(targetFile, "utf8");
  let ast;
  try {
  ...
  let modified = false;
  let oldValue = null;
  if (change.isInlineArray && change.variableLine) {
    traverse(ast, {
      ArrayExpression(nodePath) {
        if (modified) return;
        if (nodePath.node.loc?.start.line !== change.variableLine) return;
        const parent = nodePath.parentPath;
        if (!parent?.isMemberExpression() || !parent.parentPath?.isCallExpression()) return;
        const elements = nodePath.node.elements;
        if (change.arrayIndex == null || change.arrayIndex < 0 || change.arrayIndex >= elements.length) return;
        const element = elements[change.arrayIndex];
        if (!element) return;
        if (change.propertyPath && t.isObjectExpression(element)) {
    ...
    traverse(ast, {
      VariableDeclarator(nodePath) {
        ...
        if (!init) return;
        if (change.arrayIndex !== void 0) {
          if (t.isArrayExpression(init)) {
            const elements = init.elements;
            if (change.arrayIndex < 0 || change.arrayIndex >= elements.length) return;
            const element = elements[change.arrayIndex];
            if (!element) return;
          ...
          } else if (t.isObjectExpression(init) && change.propertyPath) {
            ...
            for (let i = 0; i < parts.length; i++) {
              if (t.isObjectExpression(current)) {
                ...
                if (t.isArrayExpression(prop.value)) {
                  ...
                  if (change.arrayIndex < 0 || change.arrayIndex >= elements.length) break;
                  const element = elements[change.arrayIndex];
                  if (!element) break;
                  const remainingPath = parts.slice(i + 1).join(".");
                  if (remainingPath && t.isObjectExpression(element)) {
                    const result = modifyObjectProperty(element, remainingPath, change.newValue, t);
                    if (result.success) {
                      oldValue = result.oldValue;
                      modified = true;
                    }
  ...
  if (!modified) {
    return { success: false, error: change.isInlineArray ? `Inline array not found at line ${change.variableLine}` : `Variable "${change.variableName}" not found or not editable` };
  }
  ...
  import_fs4.default.writeFileSync(targetFile, code, "utf8");
  return {
    success: true,
    file: targetFile,
    oldValue,
...
// src/server/jsx-edit.ts
var import_fs5 = __toESM(require("fs"));
var import_path4 = __toESM(require("path"));

// src/server/git.ts
var import_child_process = require("child_process");
function commitChange(filePath, messagePrefix) {
  const timestamp = Date.now();
  try {
    (0, import_child_process.execSync)(
      `git -c user.name="visual-edit" -c user.email="support@emergent.sh" add "${filePath}"`
    );
    (0, import_child_process.execSync)(
      `git -c user.name="visual-edit" -c user.email="support@emergent.sh" commit -m "${messagePrefix}_${timestamp}"`
    );
  } catch (gitError) {
...
function getElementName(nameNode) {
  ...
  if (nameNode?.property?.name) return nameNode.property.name;
  return void 0;
}
function processJsxEdits(changesByFile, frontendRoot, babelTools) {
  const { parser, traverse, generate, t } = babelTools;
  const edits = [];
  ...
  Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
    ...
    if (!targetFile) {
      targetFile = import_path4.default.resolve(frontendRoot, "src/components", `${fileName}.js`);
    }
    validateFilePath(targetFile, frontendRoot, fileName);
    if (!import_fs5.default.existsSync(targetFile)) {
      throw new Error(`File not found: ${targetFile}`);
    }
    const currentContent = import_fs5.default.readFileSync(targetFile, "utf8");
    ...
    traverse(ast, {
      JSXOpeningElement: (astPath) => {
        ...
        const elementName = getElementName(astPath.node.name);
        if (!elementName) return;
        const consumed = /* @__PURE__ */ new Set();
        changesAtLine.forEach((change, idx) => {
          if (elementName !== change.component) return;
          if (change.column !== void 0 && astPath.node.loc?.start.column !== change.column) return;
          consumed.add(idx);
          if (change.type === EDIT_TYPE.CLASS_NAME && change.className !== void 0) {
            processClassNameChange(astPath, change, targetFile, frontendRoot, edits, t);
          } else if (change.type === EDIT_TYPE.TEXT_CONTENT && (Array.isArray(change.textParts) || change.textContent !== void 0)) {
            processTextContentChange(astPath, change, targetFile, frontendRoot, edits, t);
          } else if (change.type === EDIT_TYPE.CONTENT && change.content !== void 0) {
            processContentChange(astPath, change, targetFile, frontendRoot, edits, parser, generate, t);
          } else {
            const reason = `Change must have valid type ('className', 'textContent', or 'content'). Received type: ${change.type || "undefined"}`;
            rejectedChanges.push({
              change,
              reason,
              file: getRelativePath(targetFile, frontendRoot),
              lineNumber,
              element: elementName
            });
        ...
        if (consumed.size > 0) {
          changesByLine[lineNumber] = changesAtLine.filter((_, idx) => !consumed.has(idx));
          if (changesByLine[lineNumber].length === 0) delete changesByLine[lineNumber];
        }
      }
    });
    const { code } = generate(ast, {
    ...
    if (import_fs5.default.existsSync(targetFile)) {
      const originalContent = import_fs5.default.readFileSync(targetFile, "utf8");
      import_fs5.default.writeFileSync(backupFile, originalContent, "utf8");
    }
    import_fs5.default.writeFileSync(targetFile, code, "utf8");
    commitChange(targetFile, "visual_edit");
    if (import_fs5.default.existsSync(backupFile)) {
      import_fs5.default.unlinkSync(backupFile);
    }
  });
  return { edits, rejectedChanges };
}
function processClassNameChange(astPath, change, targetFile, frontendRoot, edits, t) {
  const lineNumber = astPath.node.loc?.start.line;
  const elementName = getElementName(astPath.node.name) || "unknown";
  console.log(`[backend] Processing className change:`, change.className);
  let classAttr = astPath.node.attributes.find(
    (attr) => t.isJSXAttribute(attr) && attr.name.name === "className"
  );
  const oldClassName = classAttr?.value?.value || "";
  if (classAttr) {
    console.log(`[backend] Updating existing className from:`, classAttr.value?.value, "to:", change.className);
    ...
    astPath.node.attributes.push(newClassAttr);
  }
  edits.push({
    file: getRelativePath(targetFile, frontendRoot),
    lineNumber,
    element: elementName,
    type: EDIT_TYPE.CLASS_NAME,
    oldData: oldClassName,
    newData: change.className
  });
}
function processTextContentChange(astPath, change, targetFile, frontendRoot, edits, t) {
  const lineNumber = astPath.node.loc?.start.line;
  const elementName = getElementName(astPath.node.name) || "unknown";
  console.log(`[backend] Processing textContent change:`, change.textContent);
  const parentElementPath = astPath.parentPath;
  if (!parentElementPath || !parentElementPath.isJSXElement()) return;
  const jsxElementNode = parentElementPath.node;
  const children = jsxElementNode.children || [];
  const hasStaticText = children.some(
    (child) => t.isJSXText(child) && child.value && child.value.trim()
  );
  const hasExpressions = children.some(
    (child) => t.isJSXExpressionContainer(child) && !t.isJSXEmptyExpression(child.expression)
  );
  if (!hasStaticText && hasExpressions) {
    console.log(`[backend] Skipping textContent edit \u2014 element has no static text, only expressions`);
    return;
  }
  const textParts = Array.isArray(change.textParts) ? change.textParts : null;
  if (textParts) {
    const textLikeNodes = children.filter(
      (child) => t.isJSXText(child) && child.value && child.value.trim()
    );
    const oldContent = textLikeNodes.map((child) => child.value || "").join("");
    textLikeNodes.forEach((child, index) => {
      const nextValue = textParts[index] ?? "";
      child.value = preserveWhitespace(child.value, nextValue);
    });
    if (textParts.length > textLikeNodes.length) {
      const extraNodes = textParts.slice(textLikeNodes.length).map((part) => t.jsxText(part ?? ""));
    ...
    if (firstTextNode) {
      ...
      oldContent = "";
      fallbackWhitespaceNode.value = preserveWhitespace(fallbackWhitespaceNode.value, newContent);
    } else {
      oldContent = "";
...
function processContentChange(astPath, change, targetFile, frontendRoot, edits, parser, generate, t) {
  const lineNumber = astPath.node.loc?.start.line;
  const elementName = getElementName(astPath.node.name) || "unknown";
  console.log(`[backend] Processing content-only change:`, change.content.slice(0, 100));
  const parentElementPath = astPath.parentPath;
  if (!parentElementPath || !parentElementPath.isJSXElement()) return;
  ...
  const newChildren = parseJsxChildren(change.content, parser, t);
  if (newChildren) {
  ...
  edits.push({
    ...
    lineNumber,
    element: elementName,
    type: EDIT_TYPE.CONTENT,
    oldData: oldContent,
    newData: change.content
  });
}

// src/server/edit-route.ts
function handleEditFile(req, res) {
  const { changes } = req.body;
  if (!changes || !Array.isArray(changes) || changes.length === 0) {
    sendJson(res, 400, { error: "No changes provided" });
    return;
  }
  try {
    const edits = [];
    ...
    const t = require_lib3();
    const frontendRoot = process.cwd();
    const babelTools = { parser, traverse, generate, t };
    const variableEditChanges = changes.filter((c) => c.type === EDIT_TYPE.VARIABLE_EDIT);
    const otherChanges = changes.filter((c) => c.type !== EDIT_TYPE.VARIABLE_EDIT);
    for (const change of variableEditChanges) {
      ...
      if (!validation.valid) {
        rejectedChanges.push({ change, reason: validation.error });
        continue;
      }
      const result = processVariableEdit(change, frontendRoot, babelTools);
      if (result.success) {
        edits.push({
          file: getRelativePath(result.file, frontendRoot),
          type: EDIT_TYPE.VARIABLE_EDIT,
          variableName: change.variableName,
          arrayIndex: change.arrayIndex,
          propertyPath: change.propertyPath,
        ...
        commitChange(result.file, "visual_edit_variable");
      } else {
        rejectedChanges.push({ change, reason: result.error });
      }
    }
    const changesByFile = {};
    ...
    if (Object.keys(changesByFile).length > 0) {
      const jsxResult = processJsxEdits(changesByFile, frontendRoot, babelTools);
      edits.push(...jsxResult.edits);
      rejectedChanges.push(...jsxResult.rejectedChanges);
    }
    const response = {
      status: "ok",
      edits
...
function createMiddleware() {
  return (req, res, next) => {
    const path5 = pathname(req);
    if (req.method === "GET" && path5 === "/ping") {
      sendJson(res, 200, { status: "ok", time: (/* @__PURE__ */ new Date()).toISOString() });
      return;
    }
    if (path5 === "/edit-file") {
...
var index_default = setupDevServer;
